<div class="footer-wrapper">
    <div class="container">
        <div class="footer">
            <a href="{{ setting('site.instagram') }}" target="_blank">
                <img class="img-fluid img-thumbnail" src="{{ asset('storage/images/icon_Insta.svg') }}" 
                    alt="Instagram" loading="lazy" ALIGN=CENTER>
            </a>
            <a href="{{ setting('site.linkedin') }}" target="_blank">
                <img class="img-fluid img-thumbnail" src="{{ asset('storage/images/icon_Linkedin_.svg') }}" 
                    alt="LinkedIn" loading="lazy" ALIGN=CENTER>
            </a>
            <a href="mailto:{{ setting('site.email') }}">
                <img class="img-fluid img-thumbnail" src="{{ asset('storage/images/icon_envelope.svg') }}" 
                    alt="Email" loading="lazy" ALIGN=CENTER>
            </a>
            <a href="{{ setting('site.behance') }}" target="_blank">
                <img class="img-fluid img-thumbnail" src="{{ asset('storage/images/icon_behance_.svg') }}" 
                    alt="Behance" loading="lazy" ALIGN=CENTER>
            </a>
        </div>
    </div>
</div>